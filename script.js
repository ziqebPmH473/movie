        ...
        analysisSettings: {
            market_todayjp: {
                ...
                copyTexts: {
                    ...
                    menberinfografic: `{{today}}の市場データに基づき、上昇率トップ5、下落率トップ5を表形式でまとめてください。\n\n**【形式の指定】****「上昇率トップ5」**と**「下落率トップ5」**の左右に並べて記載すること。\n* 情報過多を防ぐため、簡潔にまとめること。\n\n**【記載項目】**\n各銘柄について、以下の4項目を必ず記載すること。\n1.  **順位**（1から5）\n2.  **銘柄名**\n3.  **銘柄コード**（4桁）\n4.  **変動率（前日比%）**\n\n**【データの正確性に関する指示】**\n* 銘柄名は正式な名称を記載すること。`
                }
            },
            dynamic_stocks: {
                label: '動的銘柄',
            ...
            market_earnings: {
                ...
                copyTexts: {
                    ...
                    presentationEarningsOnly: "{{companyName}}の決算内容を分析する{{reportSs}}{{reportGc}}",
                    thumbnail: "{{intro}}する{{thumbnail}}",
                    menberinfografic: "{{companyName}}の【累積の業績】に関する決算短信データに基づき、以下の条件を満たす予実対比表を作成してください。\n\n**【形式の指定】**\n* レイアウトは**表形式**とし、**縦長になるよう**に設計すること。\n* 情報過多を防ぐため、「簡潔」にまとめ、**通期予想と実績の比較に特化**すること。\n\n**【必須記載項目】**\n1.  **項目名**（売上高、営業利益、経常利益、純利益の4項目を優先）\n2.  **通期予想**（会社が**前回発表した通期予想値**）\n3.  **今回実績**（今回発表された**実績値**）\n4.  **進捗率または達成率**（【今回実績 ÷ 通期予想】の計算結果を**パーセンテージ**で表示すること）\n\n※業績予想が非開示の場合は、通期予想の代わりに**『前年同期の実績値』**を記載し、進捗率または達成率 の代わりに **成長率**（【今回実績 ÷ 前年同期の実績値】の計算結果を**パーセンテージ**で表示すること）して記載すること",
