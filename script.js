...
const App = {
    ...
    CONFIG: {
        ...
        commonTemplates: {
            ...
            CommonNote_source: "\n・出典や引用は一切記載しないこと。出典や引用についての、リンク・カッコ・番号などはすべて削除してください。（出典・引用以外は、カッコや番号を使用して問題ない）",//chatGPT分析
...
上期：かみき\n下期：しもき\nEBITDA：イービットディーエー\nEBITA：イービットエー\nIFRS：イファース\n終値：おわりね\n初値：はつね\n老舗：しにせ\n月次：げつじ\n高市：たかいち`,
            ...
            VoiceNote_kaihi: `\n【音声の最後のルール】\n・音声の一番最後は、必ず「本日はここまでです」の１文で終えてください。\n・１文の直後に2秒程度の無音の間を置いてから終了する（無音の指示自体は読み上げない）`,
            VoiceNote_Ks1: `ただし、決算資料のみ決算資料であることに触れて構いません。`,
            VoiceNote_Ks2: `\n・業績予想の増減表現\n音声では「上方修正」「下方修正」という言葉は使わず、予想やガイダンスが増えた場合は「引き上げた」、減った場合は「引き下げた」と表現すること。\nIRや資料に「上方修正」「下方修正」と書かれていても、読み上げる際は「業績予想を引き上げた」「業績予想を引き下げた」に言い換えること。`,
            ...
            VoiceNote_Size: `\n【長さ】\n・最終的な音声の長さが**{{voiceTime}}**になるようにしてください。日本語で{{voiceChars}}の原稿にしてください。\n・オープニング：{{op_voiceChars}}字以内、エンディング：{{end_voiceChars}}字以内\n【禁止】\n・「あなたに考えてほしいこと」「いかがでしたか」「コメントで教えて」等の問いかけ系アウトロは禁止\n・まとめ、要約は本文の最後に1回だけ。章末や本編末に追加の要約を作らない（=二重まとめ禁止）`,
        ...
        uiTemplates: {
            ...
            'youtube-posting-area': `
                <div id="youtube-posting-area">
                    ...
                    <div class="input-group" style="margin-top: 0.8em;">
                        <label>内容:</label>
                        <input type="text" id="youtube-content" placeholder="動画内容" style="width: 400px;">
                    </div>
                    <div class="input-group" style="margin-top: 0.8em;">
                        <label>概要欄:</label>
                    ...
                    <div class="input-group" style="margin-top: 0.8em;">
                        <button type="button" onclick="App.clearYoutubeFields()" style="font-size: 0.8em;">全てクリア</button>
                    </div>
                </div>
            `,
            'jpx-button': ``,
        ...
        analysisSettings: {
            market_morningsession: {
                ...
                buttonData: [
                    { category: "【ランキング作成】", services: [{ service: "notebookLM", buttons: [{ label: "URLコピー", copyId: "urls" }, { label: "RANK", copyId: "rank" }, { label: "投稿", copyId: "Notify" },{ label: "画像", copyId: "grafic" }] }] },
                    //{ category: "【ランキング作成】", services: [{ service: "notebookLM", buttons: [{ label: "URLコピー", copyId: "urls" }, { label: "RANK", copyId: "rank" }, { label: "タイトル・概要欄", copyId: "title" }] }] },
                ...
                copyTexts: {
                    ...
                    presentation: `表紙、まとめのスライドは不要\n以下に記載の内容以外を出力しないこと（以下に記載のない、説明なども一切不要）\n\n【出力内容】\n以下のスライドを作成\n・指数の値動き（大きな数字のレイアウト）\n・上昇率TOP5（表形式）\n・下落率TOP5（表形式）\n・業種別騰落率上位（表形式）\n・業種率騰落率下位（表形式）\n\n【出力形式の注意点】\n・全スライド\n順位の記載は不要\n画像は不要\n表内の文字はすべて黒字\n表の背景色は白のみ（交互に色を変えない）\n・指数\n小数点以下2桁まで記載\n・上昇率TOP5、下落率TOP5\n銘柄名、株価、前日比の列を出力\n株価は半角6文字、前日比は半角10文字が収まるぎりぎりの列幅にすること\n前日比は、変動幅(変動率)の形式で出力し、原則として同一セル内で改行しないこと。セル幅からはみ出す場合にのみ改行を許可する\n・業種別騰落率上位、業種率騰落率下位\n業種、変動率、PERの列を出力\n変動率は半角10文字、PERは半角10文字がギリギリ収まる列幅にすること`,
                    //grafic: `以下の【市場データ】に基づき、スマートフォンで見やすい「株式市場サマリー画像」を作成してください。\nAIによるデザインの自己判断を一切排除するため、以下の【描画ルール】を厳守してください。\n銘柄名は銘柄名一覧表の「銘柄名（正式）」を使用してください。ただし、銘柄名一覧表にない場合は、ソースの名称もしくはレポートの名称を使用してください。\n\n【1. 背景透かし（Watermark）の重要指定】\n* 画像全体の背景に、テキスト「@sumikko_money」を配置すること。\n* **配置方法**:\n    * 画像の中央に、非常に大きなフォントサイズで配置する。\n    * 斜め（約45度回転）に傾ける。\n    * **透かしの文字色は「極めて薄いグレー（例: #F5F5F5）」または「透明度を高く（例: opacity 0.1）」設定し、メインの文字やグラフの可読性を絶対に妨げないようにすること。あくまで背景の模様として見えるレベルに留める。\n\n【2. カラーコード定数（絶対厳守）】\n* 背景色: #FFFFFF (完全な白)\n* 文字色: #000000 (完全な黒)\n* 上昇色: #D32F2F (濃い赤)\n* 下落色: #008000 (濃い緑)\n* 枠線色: #CCCCCC (薄いグレー)\n\n【3. テキスト表記フォーマット（鉄の掟）】\nAIによる勝手な省略や結合（スラッシュ区切り等）は禁止。以下の行構成を守ること。\n\n**(A) 主要指数エリア（日経平均・TOPIX）の表記**\n* 左右に均等配置し、以下の3行構成にする。\n    * **1行目**: 指数名（例: 日経平均株価）（黒・太字・中サイズ）\n    * **2行目**: 【現在値】円/pt （黒・太字・**特大サイズ**）\n    * **3行目**: 前日比: 【符号】【変動幅】 (【符号】【変動率】%)\n        * 例: 前日比: -301.48 (-0.58%)\n        * ※この行を、上昇なら「赤」、下落なら「緑」で色付けする。\n\n**(B) 個別銘柄カードの表記**\n* **1行目**: 順位. 銘柄名（黒・太字）\n* **2行目**: 【符号】【変動率】% （例: +21.08%）\n    * ※カード内で最大フォントにし、赤/緑で色付け。強調の主役とする。\n* **3行目**: 株価: 【現在値】円 （黒・小文字）\n* **4行目**: 前日比: 【符号】【変動幅】 （黒・小文字）\n\n**(C) 業種別カードの表記**\n* **1行目**: 順位. 業種名（黒・太字）\n* **2行目**: 【符号】【変動率】% （例: +2.38%）\n    * ※カード内で最大フォントにし、赤/緑で色付け。\n* **3行目**: PER: 【数値】倍 （黒・小文字）\n\n【4. レイアウト構成】\n1. **タイトル**: 最上部に1行。「1分でわかる！今日の株式市場 (YYYY/MM/DD 前引け)」\n2. **主要指数**: 上記(A)のフォーマットで描画。\n3. **ランキング**: 上記(B)(C)のフォーマットに従い、以下の順で「横一列のカード形式（枠線付き）」で並べる。\n    * 値上がり率 TOP 5\n    * 値下がり率 TOP 5\n    * 業種別騰落率 トップ5\n    * 業種別騰落率 ワースト5`,
            ...
            market_todayjp: {
                ...
                buttonData: [
                    ...
                    { category: "【分析】", services: [{ service: "chatGPT", buttons: [{ label: "*理由ニュース", copyId: "top10ReasonNewsNG" }] }] },
                    { category: "【音声生成前】", services: [{ service: "notebookLM", buttons: [ { label: "音声生成", copyId: "voiceNew" },{ label: "画像（まとめ）", copyId: "infografic" },{ label: "ツール文章", copyId: "presen1" },{ label: "全情報出力", copyId: "reportKk" }] }] },
                    { category: "【資料用画像作成】", services: [{ service: "gemini", buttons: [ { label: "画像(表紙)", copyId: "photoTop" },{ label: "*画像(ニュース)", copyId: "photoNews" }, { label: "*X・サムネ", copyId: "xNotify" }] }] },
                ...
                copyTexts: {
                    urls: `https://quote.nomura.co.jp/nomura/cgi-bin/quote.cgi?template=nomura_tp_index_01\nhttps://s.kabutan.jp/warnings/nk225_contrib/?direction=desc&order=contrib_price\nhttps://s.kabutan.jp/warnings/nk225_contrib/?direction=asc&order=contrib_price\nhttps://www.nikkei.com/marketdata/ranking-jp/price-rise/?market=G_TP\nhttps://www.nikkei.com/marketdata/ranking-jp/price-drop/?market=G_TP\nhttps://s.kabutan.jp/warnings/sector_stocks_ranking/\nhttps://s.kabutan.jp/warnings/sector_stocks_ranking/?direction=asc&order=prev_price_ratio\nhttps://www.nikkei.com/marketdata/ranking-jp/trading-value/?market=all`,
                    rank: `東証プライム市場の上昇率・下落率ランキングTOP5の銘柄の証券コード、銘柄名、株価、株価変動幅、株価変動率を以下の形式で出力してください。\n一覧のタイトル・一覧以外の内容は一切記述しないでください。\n\n上昇率TOP5\n（上昇率上位の表）\n下落率TOP5\n（下落率上位の表）`,
                    namerank: `寄与度ランキング（トップ、ワースト）、売買代金ランキング、株価上昇率／株価下落率ランキングに記載の銘柄について\n証券コード、銘柄名を表形式で出力してください。（重複可）\n一覧表以外の内容は一切記述しないでください。\n・銘柄は各ランキングの上位5銘柄までを出力してください。`,
                    //top10ReasonNews: `１．東証プライム市場の上昇率・下落率ランキングTOP5の銘柄について、変動率が高かった具体的な理由を、各銘柄のコードや企業名で個別に調査し、以下の要因を含めて確認できた場合のみ記載してください。\n・企業のプレスリリース（新製品、提携、M&A、業績修正など）\n・決算発表\n・証券会社による投資判断や目標株価の変更\n・大株主の大量保有報告書や保有目的の変更\n・関連市場や業界全体に影響を与える重要なニュース\n・ランキングは以下のURLから取得すること。\nhttps://kabutan.jp/warning/?mode=2_1&market=1&dispmode=normal\nhttps://kabutan.jp/warning/?mode=2_2&market=1&dispmode=normal\n・変動理由が「具体的なニュース・決算発表等」で**確認できた銘柄のみ**を掲載すること。  \n・理由が未確認、あるいは{{today}}から一週間以上前の材料しかない場合は、**一切表示しない**。  \n・理由は{{today}}の15:29までに発表されたものに限定すること。\n・「理由が見つからない」「市場全体の調整」などの曖昧な記載は**禁止**。  \n・除外した銘柄は名前やコードも含めて**一切報告しない**。\n・ニュースは日本語・英語いずれでも構いません。\n・レポート形式ではなく、以下の形式でまとめてください。\n・銘柄名は証券コードから正式な名称（株式会社は除く）を調べて記載してください。\n以下に記載のない余計な文章や表などは追加しないでください。\n\n急騰・急落銘柄の材料\n\nトヨタ自動車（7203）：-10.15％\n理由となる出来事：〇〇〇〇〇〇\n日付：YYYY/M/D\n詳細：XXXXXXXXXXXXX\n\nソニー（7211）：+9.15％\n理由となる出来事：〇〇〇〇〇〇\n日付：YYYY/M/D\n詳細：XXXXXXXXXXXXX\n\n２．{{today}}の東京株式市場に大きく影響を与えたニュースをまとめて\n個別株や指数の値動きについての記載はせず、ニュースの内容に文字数を割いて\n{{prevBizDay}}の15時30分から{{today}}の15時29分の間に発表されたニュースのみ記載して\nニュースの数は任せるが、多くても５～６個まで\n・個別株の値動きのまとめや個別株の決算は除く、ただし複数銘柄に影響を与えたニュースは可\n・日本株の指数の値動きに関するまとめは除く\n・レポート形式ではなく、以下の形式でまとめてください。以下に記載のない余計な文章や表などは追加しないでください。\n\n{{prevBizDay}}の15時30分から{{today}}の15時29分の間に発表された東京株式市場に影響を及ぼしたニュース\n\n概要：〇〇〇〇〇〇\n日付：YYYY年M月D日\n詳細：XXXXXXXXXXXXX\n\n概要：〇〇〇〇〇〇\n日付：YYYY年M月D日\n詳細：XXXXXXXXXXXXX\n\n・記載するのは上記１と２の内容のみにしてください。上記以外の内容（分析やまとめなど）は不要です。\n・出典、出典のリンクは削除してください`,
                    top10ReasonNewsNG: `１．東証プライム市場の上昇率・下落率ランキングTOP5の銘柄について、変動率が高かった具体的な理由を、各銘柄のコードや企業名で個別に調査し、以下の要因を含めて確認できた場合のみ記載してください。\n・企業のプレスリリース（新製品、提携、M&A、業績修正など）\n・決算発表\n・証券会社による投資判断や目標株価の変更\n・大株主の大量保有報告書や保有目的の変更\n・関連市場や業界全体に影響を与える重要なニュース\n・ランキングは以下を使用すること。\n{{textbox}}\n・変動理由が「具体的なニュース・決算発表等」で**確認できた銘柄のみ**を掲載すること。  \n・理由が未確認、あるいは{{today}}から一週間以上前の材料しかない場合は、**一切表示しない**。  \n・理由は{{today}}の15:29までに発表されたものに限定すること。\n・「理由が見つからない」「市場全体の調整」などの曖昧な記載は**禁止**。  \n・除外した銘柄は名前やコードも含めて**一切報告しない**。\n・ニュースは日本語・英語いずれでも構いません。\n・レポート形式ではなく、以下の形式でまとめてください。\n以下に記載のない余計な文章や表などは追加しないでください。\n\n急騰・急落銘柄の材料\nトヨタ自動車（7203）：-10.15％\n理由となる出来事：〇〇〇〇〇〇\n日付：YYYY/M/D\n詳細：XXXXXXXXXXXXX\n\nソニー（7211）：+9.15％\n理由となる出来事：〇〇〇〇〇〇\n日付：YYYY/M/D\n詳細：XXXXXXXXXXXXX\n\n２．{{today}}の東京株式市場に大きく影響を与えたニュースをまとめて\n個別株や指数の値動きについての記載はせず、ニュースの内容に文字数を割いて\n{{prevBizDay}}の15時30分から{{today}}の15時29分の間に発表されたニュースのみ記載して\nニュースの数は任せるが、多くても５～６個まで\n・個別株の値動きのまとめや個別株の決算は除く、ただし複数銘柄に影響を与えたニュースは可\n・日本株の指数の値動きに関するまとめは除く\n・レポート形式ではなく、以下の形式でまとめてください。以下に記載のない余計な文章や表などは追加しないでください。\n\n{{prevBizDay}}の15時30分から{{today}}の15時29分の間に発表された東京株式市場に影響を及ぼしたニュース\n\n概要：〇〇〇〇〇〇\n日付：YYYY年M月D日\n詳細：XXXXXXXXXXXXX\n\n概要：〇〇〇〇〇〇\n日付：YYYY年M月D日\n詳細：XXXXXXXXXXXXX\n\n・記載するのは上記１と２の内容のみにしてください。上記以外の内容（分析やまとめなど）は不要です。\n・出典、出典のリンクは削除してください`,
                    voiceNew: `指数動向、各ランキングから今日の東証相場を振り返り、特徴や投資家心理、市場の注目ポイントを2分超の長さで、以下の構成で解説してください。\n個人的な感想は添えず、事実を伝えることに専念してください。また、メタ情報は発言しないようにしてください。\n【構成】\n1.指数の値動き\n2.日経平均寄与度\n3.業種別の値動き\n4.個別銘柄の値動き（上昇率ランキング、下落率ランキング）\n5.売買代金ランキング\n6.ニュース{{VoiceNote_Principle}}{{VoiceNote_Basic}}{{nameConvert}}{{VoiceNote_Ks}}\n・選別、まだら模様などの表現は禁止(市場はそういうもので当たり前のこと、全銘柄が一方向に動くものではないため){{VoiceNote_tatoe}}{{VoiceNote_kaihi}}`,
                    infografic: `この日の東証マーケットを振り返る動画の最後に「まとめ」として表示する、東証マーケットのまとめ画像を作成してください。\n全ての情報を記載する必要はありませんが、この日のマーケットの特長がわかるようにしてください。\n背景は白を基調としてください。`,
                    presen1: `以下の形式で、表紙（1行目のタイトルはそのまま、2行目以降が本文）、指数、値上がり値下がり、寄与度、業種別騰落率、ニュースを出力してください。\n表紙の本文は、改行せずに、140文字程度でこの日の市場を簡潔にまとめてください。\n銘柄名は銘柄名一覧表の「銘柄名（正式）」を使用してください。\nただし、銘柄名一覧表にない場合は、ソースの名称もしくはレポートの名称を使用し、「ホールディングス」はＨＤ、「フィナンシャルグループ」はＦＧ、「グループ」はＧに変換してください。\nニュースの見出しは30文字（60バイト）以内、ニュースの詳細は90文字（180バイト）以内にまとめてください。\nニュースは最大5本までとし、東京株式市場に影響を及ぼしたニュースから、特に市場への影響が大きかったと思われるものを選定してください。ただし、ニュースの本数が4本に満たない場合に限り、個別銘柄の急騰・急落銘柄の材料を加えて、合計4本としてください。\n\n\n日付: 2026/03/15\n\n[表紙]\nマーケット振り返り\n2026年3月15日の日本株式市場は全体的に上昇。\n日経平均は+170円と小幅続伸。\n\n[指数]\n日経平均|58,753.39|+170.27|+0.29%\nTOPIX|3,880.34|+37.18|+0.97%\n東証スタンダードTOP20|1,612.59|+2.74|+0.17%\n東証グロースCore|875.24|+15.98|+1.86%\n\n[上昇率]\n東邦チタニウム|5727|2,291|+400|+21.15%\nテクセンドフォトマスク|429A|3,375|+479|+16.54%\nＳＨＩＦＴ|3697|691.2|+87.2|+14.44%\nAppier G|4180|818|+96|+13.30%\nユニチカ|3103|1,617|+189|+13.24%\n\n[下落率]\n楽天銀行|5838|6,878|-1,080|-13.57%\n松屋|8237|2,548|-318|-11.10%\nヨコオ|6800|3,415|-330|-8.81%\n日東紡績|3110|25,470|-2,210|-7.98%\n大和冷機工業|6459|1,804|-151|-7.72%\n\n[寄与度]\nソフトバンクG|9984|+129.16\nファナック|6954|+46.46\nリクルートHD|6098|+46.13\nＴＤＫ|6762|+26.32\nフジクラ|5803|+21.39\n---\nアドバンテスト|6857|-131.03\n東京エレクトロン|8035|-92.25\nダイキン工業|6367|-27.74\nイビデン|4062|-23.60\n住友電気工業|5802|-15.88\n\n[売買代金]\nキオクシアHD|285A|21,240|-225|-1.05%|5,946億円\nフジクラ|5803|27,465|+640|+2.39%|4,425億円\nアドバンテスト|6857|28,125|-490|-1.71%|3,246億円\nＪＸ金属|5016|4,142|-3|-0.07%|3,189億円\nディスコ|6146|79,070|-930|-1.16%|2,103億円\n\n[業種]\nサービス業|+3.29%|20.4倍\n銀行業|+3.28%|15.3倍\n海運業|+2.81%|10.9倍\n情報・通信業|+2.60%|26.3倍\n保険業|+2.36%|18.3倍\n---\n小売業|-1.39%|33.6倍\nガラス・土石|-1.02%|76.6倍\n非鉄金属|-0.87%|26.6倍\n食料品|-0.63%|28.8倍\n鉄鋼|-0.24%|16.8倍\n\n[ニュース]\nエヌビディア　11-1月期決算|純利益前年同期比+94%、売上高+73%でいずれも過去最高。2-4月期見通し約780億ドル。\n楽天G　フィンテック事業再編協議再開|楽天銀行・カード・証券をHDとして集約する組織再編を想定し、協議再開を公表。\n高市首相　IMF提言・消費税・日銀政策に言及|飲食料品の消費税2年間停止案を一定評価。日銀政策について2%物価目標の安定実現に向けた運営に期待。\n内閣府　12月景気動向指数改定値|一致指数前月比0.6ポイント低下の114.3。基調判断「足踏みを示している」を据え置き。`,
                    reportKk: `各サイトのデータからは指数、各ランキング、レポートからは急騰・急落銘柄の材料、東京株式市場に影響を及ぼしたニュースをまとめて出力してください。\n・急騰・急落銘柄の材料、東京株式市場に影響を及ぼしたニュースは、レポートに記載の内容をそのまま出力してください。ただし、銘柄名は「銘柄名一覧表」の「銘柄名（正式）」に置き換えてください。\n・指数は銘柄名、株価、前日比（日経平均、TOPIX、東証スタンダード、東証グロース）\n・各市場（プライム、スタンダード、グロース）の出来高・時価総額\n・寄与度ランキング（トップ、ワースト）は銘柄名、証券コード、寄与度\n・業種別ランキング（トップ、ワースト）は業種名、変動率、PER\n・売買代金ランキングは銘柄名、証券コード、株価、前日比、売買代金\n・株価上昇率／株価下落率ランキングは銘柄名、証券コード、株価、前日比\n※前日比は「変動幅（変動率）」形式、ランキングは上位5位まで\n※業種別ランキングは、変動率がマイナスでも上位5位まで、変動率がプラスでも下位5位まで記載する。\n※ランキングや指数はいつ時点の情報かを記述しない\n・まとめ→今日の東証相場を振り返り、特徴や投資家心理、市場の注目ポイントのまとめ\n{{nameConvert}}`,
                    xNotify: "以下の資料から２分で{{intro}}動画のサムネイルの文字と、Xへの投稿文を作成してください。\n\n{{textbox}}",
                    photoTop: `「東証マーケットを振り返る」をイメージして画像を生成してください。\n文字や企業ロゴは記載しないでください。`,
                    photoNews: `以下の中に記載のニュースの中から1つをピックアップし、そのニュースをイメージした画像を生成してください。\n文字や企業ロゴは記載しないでください。\n\n{{textbox}}`,
                    //voiceNew: `指数動向、各ランキングから今日の東証相場を振り返り、特徴や投資家心理、市場の注目ポイントを以下の構成で解説してください。\n【構成】\n1.指数の値動き\n2.日経平均寄与度\n3.業種別の値動き\n4.個別銘柄の値動き（上昇率ランキング、下落率ランキング）\n5.売買代金ランキング\n6.ニュース{{VoiceNote_Principle}}{{VoiceNote_Read}}{{VoiceNote_Basic}}{{nameConvert}}{{VoiceNote_Ks}}{{VoiceNote_Size}}\n・選別、まだら模様などの表現は禁止(市場はそういうもので当たり前のこと、全銘柄が一方向に動くものではないため){{VoiceNote_tatoe}}{{VoiceNote_kaihi}}`,
                    voice: `指数動向、各ランキングから今日の東証相場を振り返り、特徴や投資家心理、市場の注目ポイントを分析してください。\nニュースは分析の材料として使い、ニュースの紹介をメインにしないこと。\n日経平均とTOPIXの指数の違いについては解説しないこと。（指数の数字ではなく、指数自体の説明をしないこと）{{VoiceNote_Principle}}{{VoiceNote_Read}}{{VoiceNote_Basic}}{{nameConvert}}{{VoiceNote_Ks}}{{VoiceNote_Size}}{{VoiceNote_tatoe}}{{VoiceNote_kaihi}}`
                    //reportKk: `資料作成用に指数、各ランキングなどをまとめて\n・ニュース→見出し＋内容２行以内の形式の、一覧で（テキストの資料から取得し、株探からは取得しない）\n・指数は銘柄名、株価、前日比（日経平均、TOPIX、東証スタンダード、東証グロース）\n・各市場（プライム、スタンダード、グロース）の出来高・時価総額\n・寄与度ランキング（トップ、ワースト）は銘柄名、証券コード、寄与度\n・業種別ランキング（トップ、ワースト）は業種名、変動率、PER\n・売買代金ランキングは銘柄名、証券コード、株価、前日比、売買代金\n・株価上昇率／株価下落率ランキングは銘柄名、証券コード、株価、前日比\n※前日比は「変動幅（変動率）」形式、ランキングは上位5位まで\n※業種別ランキングは、変動率がマイナスでも上位5位まで、変動率がプラスでも下位5位まで記載する。\n※ランキングや指数はいつ時点の情報かを記述しない\n・まとめ→今日の東証相場を振り返り、特徴や投資家心理、市場の注目ポイントのまとめ\n・ニュースは株探から抽出しないでください。\n・ニュースには、個別株の変動理由を記載しないでください。{{nameConvert}}`,
                    //presentation: `以下の流れで資料を作成して\n表紙\n国内主要株価指数（銘柄名、株価、前日比）\n各市場（プライム、スタンダード、グロース）の出来高・時価総額\n株価上昇率ランキング（銘柄名、コード、株価、前日比）\n株価下落率ランキング（銘柄名、コード、株価、前日比）\n日経平均プラス寄与度ランキング（銘柄名、コード、寄与度）\n日経平均マイナス寄与度ランキング（銘柄名、コード、寄与度）\n売買代金ランキング（銘柄名、コード、株価、前日比、売買代金）\n業種別変動率ランキング（上位）（業種名、変動率、PER）\n業種別変動率ランキング（下位）（業種名、変動率、PER）\nニュース\nまとめ\n\n・上記に記載のない項目は出力しないでください。\n・指数は小数点以下2桁まで記載してください\n・指数は大きな数字のレイアウトでまとめてください\n・ランキングは表形式にし、見出し行をつけ、上位5位まで記載して。\n・表の色はデフォルトにしてください。\n・前日比を記述する場合は「変動幅（変動率）」の形式で記述して\n・順位は記載しない\n・寄与度ランキングは左にプラス寄与、右にマイナス寄与で１ページにして\n・業種別ランキングは左に上位、右に下位で１ページにして\n・ランキングのタイトルは上記に記載の通りとし、後ろに日付を付けたりしないで\n・表紙は、表題（「M月D日東証マーケット振り返り」の形式）と、概要を4行程度の文章でまとめてください。\n・売買代金は億円単位で記載してください。\nただし、「この資料は」などとプレゼン資料自体の事を書かないでください\nまた、資料・動画の視聴意欲がなくならないように、結論や最も重要な内容は書かないでください。\nまとめは視覚的に理解しやすいレイアウトにしてください\n・銘柄名の「ホールディングス」は「ＨＤ」、「フィナンシャルグループ」は「ＦＧ」、グループは「Ｇ」、「ＮＥＸＴ　ＦＵＮＤＳ　日経平均レバレッジ・インデックス連動型上場投信」は「日経レバレッジ」と略してください。`,
                    //presenShort: `【出力内容】\n・表紙\n・国内主要株価指数\n出力項目：銘柄名、株価、前日比\n出力形式：指数は小数点以下2桁まで記載すること。大きな数字のレイアウトでまとめること。前日比は「変動幅（変動率）」の形式で記載すること。\n・各市場（プライム、スタンダード、グロース）の出来高・時価総額\n・上昇率ランキング\n出力項目：銘柄名、株価、前日比\n・下落率ランキング\n出力項目：銘柄名、株価、前日比\n・日経平均寄与度\n出力項目：\nプラス寄与：銘柄名、コード、寄与度\nマイナス寄与：\n銘柄名、コード、寄与度\n出力形式：上にプラス寄与、下にマイナス寄与で１ページに記載\n・売買代金ランキング\n出力項目：銘柄名、変動率、売買代金\n出力形式：売買代金は百万円単位\n・業種別ランキング\n変動率上位：\n出力項目：業種名、変動率、PER\n変動率下位：\n出力項目：業種名、変動率、PER\n出力形式：上に変動率上位、下に変動率下位で１ページに記載\n・ニュース\n・まとめ\n\n【注意点】\n・順位など、出力項目に記載のない内容は出力しないこと\n・ランキングは表形式にし、見出し行をつけ、上位5位まで記載すること。\n・「前日比」は「変動幅（変動率）」の形式で記述すること。原則として同一セル内で改行しないこと。セル幅からはみ出す場合にのみ改行を許可すること。\n・ランキングのタイトルは上記に記載の通りとし、後ろに日付を付けたりしないで\n・表紙は、表題と、概要を4行程度の文章でまとめてください。\nただし、「この資料は」などとプレゼン資料自体の事を書かないでください\nまた、資料・動画の視聴意欲がなくならないように、結論や最も重要な内容は書かないでください。\nまとめは視覚的に理解しやすいレイアウトにしてください\n・銘柄名の「ホールディングス」は「ＨＤ」、「フィナンシャルグループ」は「ＦＧ」、グループは「Ｇ」、「ＮＥＸＴ　ＦＵＮＤＳ　日経平均レバレッジ・インデックス連動型上場投信」は「日経レバレッジ」と略してください。\n・列名が「前日比」「変動率」「株価」「寄与度」「PER」の列幅は、半角12文字が収まるぎりぎりの幅にしてください。`,
                    ...
                    //grafic: `【市場データ】に基づき、スマートフォンで見やすい「株式市場サマリー画像」を作成してください。\nAIによるデザインの自己判断を一切排除するため、以下の【描画ルール】を厳守してください。\n銘柄名は銘柄名一覧表の「銘柄名（正式）」を使用してください。ただし、銘柄名一覧表にない場合は、ソースの名称もしくはレポートの名称を使用してください。\n\n【1. 背景透かし（Watermark）の重要指定】\n* 画像全体の背景に、テキスト「@sumikko_money」を配置すること。\n* **配置方法**:\n    * 画像の中央に、非常に大きなフォントサイズで配置する。\n    * 斜め（約45度回転）に傾ける。\n    * **透かしの文字色は「極めて薄いグレー（例: #F5F5F5）」または「透明度を高く（例: opacity 0.1）」設定し、メインの文字やグラフの可読性を絶対に妨げないようにすること。あくまで背景の模様として見えるレベルに留める。\n\n【2. カラーコード定数（絶対厳守）】\n* 背景色: #FFFFFF (完全な白)\n* 文字色: #000000 (完全な黒)\n* 上昇色: #D32F2F (濃い赤)\n* 下落色: #008000 (濃い緑)\n* 枠線色: #CCCCCC (薄いグレー)\n\n【3. テキスト表記フォーマット（鉄の掟）】\nAIによる勝手な省略や結合（スラッシュ区切り等）は禁止。以下の行構成を守ること。\n\n**(A) 主要指数エリア（日経平均・TOPIX）の表記**\n* 左右に均等配置し、以下の3行構成にする。\n    * **1行目**: 指数名（例: 日経平均株価）（黒・太字・中サイズ）\n    * **2行目**: 【現在値】円/pt （黒・太字・**特大サイズ**）\n    * **3行目**: 前日比: 【符号】【変動幅】 (【符号】【変動率】%)\n        * 例: 前日比: -301.48 (-0.58%)\n        * ※この行を、上昇なら「赤」、下落なら「緑」で色付けする。\n\n**(B) 個別銘柄カードの表記**\n* **1行目**: 順位. 銘柄名（黒・太字）\n* **2行目**: 【符号】【変動率】% （例: +21.08%）\n    * ※カード内で最大フォントにし、赤/緑で色付け。強調の主役とする。\n* **3行目**: 株価: 【現在値】円 （黒・小文字）\n* **4行目**: 前日比: 【符号】【変動幅】 （黒・小文字）\n\n**(C) 業種別カードの表記**\n* **1行目**: 順位. 業種名（黒・太字）\n* **2行目**: 【符号】【変動率】% （例: +2.38%）\n    * ※カード内で最大フォントにし、赤/緑で色付け。\n* **3行目**: PER: 【数値】倍 （黒・小文字）\n\n【4. レイアウト構成】\n1. **タイトル**: 最上部に1行。「1分でわかる！今日の株式市場 (YYYY/MM/DD)」\n2. **主要指数**: 上記(A)のフォーマットで描画。\n3. **ランキング**: 上記(B)(C)のフォーマットに従い、以下の順で「横一列のカード形式（枠線付き）」で並べる。\n    * 値上がり率 TOP 5\n    * 値下がり率 TOP 5\n    * 業種別騰落率 トップ5\n    * 業種別騰落率 ワースト5`,
                    //menberinfografic: `{{today}}の株価変動率の市場データに基づき、以下の要件を厳守して「SNS共有用の市場振り返りスライド画像」を1枚生成してください。\n\n【1. デザインと配色の絶対ルール】\n視認性とYoutube表示への最適化を最優先すること。\n* **背景色（Canvas）:** \n*「薄いグレー (#F2F2F2)」** ※真っ白は禁止。\n* **カード（Card）:** **「真っ白 (#FFFFFF)」**のカードに**「薄いドロップシャドウ」**を付け、背景から浮き上がらせる。\n* **余白（Padding）:** 画面下部にYoutubeのシークバーが被るため、**画像の下側には十分な余白を空けること。**コンテンツを上下左右の端ギリギリに配置しない。\n* **テーマカラー:** 上昇＝緑(#00A86B)、下落＝赤(#FF4500)。\n\n【2. カード内のレイアウト構成】\n各銘柄の情報が重ならないよう、必ず以下の**「上下2段構成」**にする。\n* **左側:** 「順位」（1位〜5位）\n* **右側:**\n    * **1行目:** 「銘柄名」 + 「(コード)」 （色は黒またはグレー）\n    * **2行目:** 「変動率%」 （色は**テーマカラーの太字**で強調）\n\n【3. 記載データ】\n以下のテキストのみを記載し、余計な装飾文は一切入れないこと。\n* **タイトル:** 「{{today}} 市場の動き：株価変動率ランキング」\n* **左カラム:** 見出し「上昇率トップ5」の下に、上記ルールで5銘柄。\n* **右カラム:** 見出し「下落率トップ5」の下に、上記ルールで5銘柄。\n
//**【データの正確性に関する指示】**\n* 銘柄名は正式な名称を記載すること。`
                }
            },
            dynamic_stocks: {
            ...
            market_term_rank: {
                ...
                buttonData: [
                    ...
                    { category: "【分析】", services: [{ service: "chatGPT", buttons: [{ label: "ニュース", copyId: "news" },{ label: "*週間理由", copyId: "analysisWeek" },{ label: "*月間理由", copyId: "analysisMonth" }] }] },                    //{ category: "【分析】", services: [{ service: "chatGPT", buttons: [{ label: "*銘柄分析(ランクNG)", copyId: "analysisRankNg" }] }] },
                    { category: "【音声生成前】", services: [{ service: "notebookLM", buttons: [{ label: "ピックアップ銘柄", copyId: "pickup" },{ label: "*音声生成", copyId: "voice" }, { label: "概要欄", copyId: "gaiyo" }, { label: "動画タイトル", copyId: "titleBf" }, { label: "動画内容", copyId: "videoContent" }, { label: "根拠資料生成", copyId: "reportKk" }] }] },
                    { category: "【プレゼン資料】（スライド：6+銘柄数）", services: [{ service: "gamma", buttons: [{ label: "*プレゼン生成", copyId: "presentation" }] }] },
                ...
                copyTexts: {
                    ...
                    news: `**記載するのは以下の手順で指定された内容のみとし、それ以外の文章（冒頭の挨拶や結びの言葉など）は一切含めないでください。**テキストをコピーして使用するため、引用元や出典は一切記載しないでください。引用元や出典についての、リンク・番号・カッコも削除してください。\n\n**指数の値動き**: **先週末（{{lastWeekFriday}}）から今週末（{{thisWeekFriday}}）まで**の期間について、以下の形式で主要指数の動きをまとめてください。指数・変動率は小数点以下2桁まで記載してください。\n    ----\n    【指数の週間動向（{{lastWeekFriday}}～{{thisWeekFriday}}）】\n    ・日経平均株価: 先週末（{{lastWeekFriday}}）終値 XXXX.XX円 → 今週末（{{thisWeekFriday}}）終値 YYYY.YY円 (前週比 ZZZ.ZZ円, A.BB%)\n    ・TOPIX: 先週末（{{lastWeekFriday}}）終値 XXXX.XX → 今週末（{{thisWeekFriday}}）終値 YYYY.YY (前週比 ZZZ.ZZ, A.BB%)\n    ----\n* **主要経済ニュース**: 先週末（{{lastWeekFriday}}）の15:30から今週末（{{thisWeekFriday}}）の15:29に日本の株式市場に影響を与えたと考えられる主要な経済ニュースやイベント（個別銘柄のニュースは除く）を3～5つ、以下の形式で挙げてください。\n    【今週の主要ニュース】\n    概要：〇〇〇〇〇〇\n    日付：YYYY年M月D日\n    詳細：XXXXXXXXXXXXX`,
                    analysisWeek: `以下の手順と情報源を基に、日本の株式市場に関する包括的な分析レポートを作成してください。このレポートは、注目銘柄を選定するための判断材料として使用します。**記載するのは以下の手順で指定された内容のみとし、それ以外の文章（冒頭の挨拶や結びの言葉など）は一切含めないでください。**テキストをコピーして使用するため、引用元や出典は一切記載しないでください。引用元や出典についての、リンク・番号・カッコも削除してください。\n\nランキング上位銘柄の分析**\n\n分析対象リスト:\n{{textbox}}\n\n「分析対象リスト」に含まれる週間{{termDirectionLabel}}率ランキングに記載のすべての銘柄について、漏れなく詳細な分析を行ってください。\n\n**分析の観点（{{strategy}}）**:\n* **{{termDirectionLabel}}要因の特定**: 株価{{termDirectionLabel}}の直接的な原因となった出来事（例：好決算、新製品発表、M&Aなど）を具体的に特定し、その日付も記載してください。ただし、{{thisWeekFriday}}までの1週間以内に発生した出来事のみを対象としてください。\n**出力形式**:\n【銘柄分析】 {{銘柄名}}（{{証券コード}}）\n {{termDirectionLabel}}要因: （ここに具体的な要因を記述）\n要因発生日：YYYY年M月D日\n {{strategy_suggestion_title}}: （ここに考察を記述）\n\n【重要ルール】\n・要因発生日は「YYYY年M月D日」の形式で記載すること「M月D日頃」や「M月上旬」などの曖昧な表現は禁止`,
                    analysisMonth: `以下の手順と情報源を基に、日本の株式市場に関する包括的な分析レポートを作成してください。このレポートは、注目銘柄を選定するための判断材料として使用します。**記載するのは以下の手順で指定された内容のみとし、それ以外の文章（冒頭の挨拶や結びの言葉など）は一切含めないでください。**テキストをコピーして使用するため、引用元や出典は一切記載しないでください。引用元や出典についての、リンク・番号・カッコも削除してください。\n\nランキング上位銘柄の分析**\n\n分析対象リスト:\n{{textbox}}\n\n「分析対象リスト」に含まれる月間{{termDirectionLabel}}率ランキングに記載のすべての銘柄について、漏れなく詳細な分析を行ってください。\n\n**分析の観点（{{strategy}}）**:\n* **{{termDirectionLabel}}要因の特定**: 株価{{termDirectionLabel}}の直接的な原因となった出来事（例：好決算、新製品発表、M&Aなど）を具体的に特定し、その日付も記載してください。ただし、{{thisWeekFriday}}までの1ヶ月以内に発生した出来事のみを対象としてください。\n**出力形式**:\n【銘柄分析】 {{銘柄名}}（{{証券コード}}）\n {{termDirectionLabel}}要因: （ここに具体的な要因を記述）\n要因発生日：YYYY年M月D日\n {{strategy_suggestion_title}}: （ここに考察を記述）\n\n【重要ルール】\n・要因発生日は「YYYY年M月D日」の形式で記載すること「M月D日頃」や「M月上旬」などの曖昧な表現は禁止`,
            ...
            news: {
                ...
                copyTexts: {
                    ...
                    voice: `{{intro}}してください。{{VoiceNote_Principle}}{{VoiceNote_Read}}{{readingNote}}{{VoiceNote_Basic}}{{VoiceNote_Ks}}{{VoiceNote_Size}}{{VoiceNote_tatoe}}{{VoiceNote_kaihi}}`,
            ...
            market_stock: {
                ...
                copyTexts: {
                    ...
                    voice: "{{intro}}を分析してください。{{VoiceNote_Principle}}{{VoiceNote_Read}}{{readingNote}}{{VoiceNote_Basic}}{{VoiceNote_Ks}}{{VoiceNote_Size}}{{VoiceNote_tatoe}}{{VoiceNote_kaihi}}\n・レポートと株探や資料で情報が異なる場合は、株探、会社発表の資料の情報を正としてください。",
...
## 分析における「3つの必須回答項目」
分析のアプローチや構成は全てあなたに一任しますが、レポートには以下の3つの問いに対する明確な答えを必ず含めてください。
...
2. （現在の株価から）どこまで{現在のトレンド}する可能性があるのか？
    * ここでいう「{中間レンジの呼称}」とは、12か月後の到達株価ではなく、{中間レンジの定義}を指します。
    * 今後の{中間レンジの呼称}は「現在の株価（本レポートの1章であなたが提示した値）」を基準（P0）に、弱気/ベース/強気の3シナリオで提示してください。
    * 各シナリオは「目標株価レンジ（価格）」と「P0からの変化率（%）」を必ず併記してください。
    * {中間レンジの制約} P0を特定できない場合は、価格の推測はせず、変化率（%）のみで提示してください。
    * この{中間レンジの呼称}とは別に、「{12か月シナリオの呼称}」を別見出し・別シナリオで必ず提示してください。両者を同じ表・同じ段落・同じシナリオとして混在させてはいけません。
...
## 出力上の重要ルール
* 「{中間レンジの呼称}」と「{12か月シナリオの呼称}」は厳密に別物として扱ってください。
* 前者は{中間レンジの定義}、後者は12か月後の到達レンジです。
* 両者を同じ弱気/ベース/強気セットとしてまとめたり、同じ表に統合したりしてはいけません。
...
#### 3. 将来予測：市場コンセンサス、{中間レンジの呼称}、12か月後シナリオ、転換点のモノサシ
（このセクションでは、以下を必ず別々の小見出しで記述してください。
1. 市場コンセンサス
2. {中間レンジの呼称}（{中間レンジの定義}）
3. {12か月シナリオの呼称}（12か月後の到達レンジ想定）
4. トレンドの転換点を判断するための「株価以外のモノサシ」
2と3は絶対に混同せず、別表または別段落で明確に分離してください。）

#### 4. シナリオとリスク
（分析の前提となるシナリオと、投資家が注意すべき最大のリスクを具体的に記述してください。特に、3章で示した「{中間レンジの呼称}」と「{12か月シナリオの呼称}」を前提に、「{リスクシナリオ}」と、12か月後シナリオ自体が下方修正されるリスクを分けて説明してください。）`,
                    analysisModes: {
                        up: {
                          '{分析タイトル}': '上昇の持続性と天井のサイン',
                          '{現在のトレンド}': '上昇',
                          '{将来のポテンシャル}': '上値余地',
                          '{転換点の意味}': '天井圏（上昇トレンドの終焉）',
                          '{投資家のゴール}': '利確・損切りポイントの特定',
                          '{リスクシナリオ}': '上昇トレンドが崩れ、下落に転じるリスク',
                          '{中間レンジの定義}': '今後3〜9か月程度の間に一時的に付けうる中間高値レンジ',
                          '{中間レンジの制約}': '目標株価レンジは必ずP0以上（P0より低い数値は禁止）',
                          '{中間レンジの呼称}': '上値余地',
                          '{12か月シナリオの呼称}': '12か月後シナリオ'
                        },
                        down: {
                          '{分析タイトル}': '下落の要因と底値のサイン',
                          '{現在のトレンド}': '下落',
                          '{将来のポテンシャル}': '下値余地',
                          '{転換点の意味}': '底値圏（下落トレンドの終焉と反転）',
                          '{投資家のゴール}': '買いポイントの特定',
                          '{リスクシナリオ}': '底打ちと判断して買ったが、さらに下落が続くリスク',
                          '{中間レンジの定義}': '今後3〜9か月程度の間に一時的に付けうる中間安値レンジ',
                          '{中間レンジの制約}': '目標株価レンジは必ずP0以下（P0より高い数値は禁止）',
                          '{中間レンジの呼称}': '下値余地',
                          '{12か月シナリオの呼称}': '12か月後シナリオ'
                        }
                    },
            ...
            market_earnings: {
                ...
                buttonData: [
                    ...
                    { category: "【プレゼン資料】", services: [{ service: "gamma", buttons: [{ label: "プレゼン生成", copyId: "presentation" }] }] },
                    { category: "【X投稿用】", services: [{ service: "notebookLM", buttons: [{ label: "chatGPT分析", copyId: "chatAnalysis" }, { label: "URLコピー", copyId: "urls" }, { label: "notebookLM出力", copyId: "notebookOut" }] }] }
                ],
                copyTexts: {
                    urls: (vars) => vars.emarket === 'jp' ? `https://kabutan.jp/stock/kabuka?code=${vars.ticker}\nhttps://kabutan.jp/stock/finance?code=${vars.ticker}` : `https://us.kabutan.jp/stocks/${vars.ticker}/\nhttps://us.kabutan.jp/stocks/${vars.ticker}/finance`,
                    voice: "{{intro}}してください。{{VoiceNote_Principle}}{{VoiceNote_Read}}{{readingNote}}{{VoiceNote_Basic}}{{VoiceNote_Ks}}{{VoiceNote_Size}}{{VoiceNote_tatoe}}{{VoiceNote_kaihi}}",
                    reportKk: `{{intro}}する{{reportKk}}{{reportKkks}}`,
                    reportKkEarningsOnly: `{{companyName}}の決算内容を分析する{{reportKk}}{{reportKkks}}`,
                    presentation: "{{intro}}する{{reportSs}}{{reportGc}}",
                    presentationEarningsOnly: "{{companyName}}の決算内容を分析する{{reportSs}}{{reportGc}}",
                    thumbnail: "{{intro}}する{{thumbnail}}",
                    menberinfografic: "{{companyName}}の**【累積の業績】**に関する決算短信データに基づき、以下の条件を満たす予実対比表を作成してください。\n\n**【形式の指定】**\n* レイアウトは**表形式**とし、**縦長になるよう**に設計してください。\n* 情報過多を防ぐため、「簡潔」にまとめ、**通期予想と実績の比較に特化**すること。\n\n【データ抽出の優先順位】\n1. **実績値:** 最新の四半期累計実績を使用。\n2. **比較対象（通期予想）:**\n   * 資料内に「業績予想の修正」に関する記載がある場合は、必ず**【修正後の数値】**を使用すること。\n   * 修正がない場合は、通常の【通期予想】を使用すること。\n   * 通期予想自体が非開示の場合は、前年同期実績を使用すること。\n\n【タイトル】\n「{{companyName}} 〇〇〇〇年〇〇期 業績進捗レポート」の形式で記述すること。\n\n**【記載項目】**\n以下の4つのデータ項目を記載し、数値は**〇〇億円**など単位を統一して分かりやすくすること。\n1.  **項目名**（売上高、営業利益、経常利益、純利益の4項目を優先）\n2.  **通期予想**（会社が発表した**通期予想値**）\n3.  **今回実績**（今回発表された**最新の実績値**）\n4.  **進捗率または成長率**\n    * ケースA（通期予想がある場合）：\n      * 進捗率の計算式：【今回実績 ÷ 通期予想】の計算結果を**パーセンテージ**にする\n      * 表示：右端に数値に対応した長さの「緑色の横棒グラフ（プログレスバー）」と進捗率を描画する。\n    * ケースB（通期予想がなく、前年同期比を使う場合）：\n      * 「前年同期実績」と比較して「前年比（％）」＝成長率を算出する。\n      * プログレスバーは描画せず、成長率の数値（％）のみを大きく表示する。\n\n【視覚化の条件分岐】\n* ケースA（通期予想がある場合）：\n  右列の進捗率（％）の背景または横に、数値に応じた長さの「横棒グラフ（プログレスバー）」を描画する。バーの色は緑色。\n* ケースB（通期予想がなく、前年同期比を使う場合）：\n  プログレスバーは描画せず、数値（％）のみを大きく表示する。\n\n**【目的】**\nこの表は、通期予想に対する実績の進捗度合いを一目で判断するための**実戦用ツール**として機能させること。",
                    //X投稿用
                    chatAnalysis: "あなたはプロの証券アナリストです。\n以下の銘柄について、ウェブ検索機能（株探、Yahoo!ファイナンス、ロイター、日経新聞など）を用いて調査し、結果をまとめてください。\n\n【対象銘柄】\n・銘柄名とコード：{{companyName}}\n・決算発表日：{{formattedEarningsDate}}{{timing}}\n・株価の反応：{{direction}}({{formattedEarningsDate}}{{afterHours}})\n\n【調査項目】\n1. 事前コンセンサスと株価変動の理由\n- 今回の決算に対する「コンセンサス（売上高、営業利益など）」を検索し、実際の数値がそれを上回ったか下回ったかを調べてください。（情報源のURLも記載）\n- 実際のニュースや市場の解説記事を検索し、「なぜ株価がそのように反応したのか（材料出尽くし、実質上方修正など）」を調査してください。\n\n2. X（Twitter）投稿用テキストの作成\n- 画像に詳細な業績数値はすべて記載されるため、投稿文に細かい数字（売上〇〇億円など）を羅列しないでください。\n- キャッチーな1行目のフック、今回の決算の「最大のポイント（なぜ株価が動いたか）」、そして「今後（次回）の注目点」に絞って、140文字程度で簡潔に作成してください。\n- ハッシュタグ（#銘柄名 #証券コード #日本株）を含めてください。\n\n【出力形式の厳守】\n以下の2つを、それぞれ別々のMarkdownのコードブロック形式（`テキスト`）で出力してください。\n※重要：コードブロックの「中」には、【ブロック2】や【X投稿用テキスト】といった見出しを絶対に書かないでください。私がコピーボタンを押したときに、コピペしてそのまま使える純粋なテキストのみを記述してください。\n\n・1つ目のコードブロック：「1. 事前コンセンサスと株価変動の理由」の調査結果\n・2つ目のコードブロック：「2. X投稿用テキスト」",
                    notebookOut: "あなたはプロの証券アナリストであり、データ整形のエキスパートです。\n読み込んだソースを総合して分析し、下記の【出力フォーマット】に厳密に従ってテキストを出力してください。\n\n## 重要なルール（必ず守ること）\n1. 決算資料から「増減益の理由」「ガイダンスの背景」を深く読み取り、reason に1〜2行で簡潔に記載してください。\n2. 金額は「兆・億・万円」の単位に変換しカンマを入れてください。（例：12,345百万円 → 123億円）\n3. 【比較ラベル】[KPI_HEADER]の consensus_label は、比較対象が市場コンセンサスなら「市場予想」、直前の会社計画なら「会社予想」と出力してください。\n4. 【コンセンサス判定】予想数値（consensus）と、結果判定（judgment：クリア、未達 など）を分けて出力してください。\n5. 【ガイダンス】今回の決算が「四半期」なら col1_header=修正前, col2_header=修正後 とし、「本決算」なら col1_header=前期実績, col2_header=今期予想 としてください。\n6. [CHART_DATA]の data は「YYYY-MM-DD,始値,高値,安値,終値;」の形式で繋げた文字列にしてください。\n7. コードブロック（```）は不要です。テキストのみ出力してください。\n\n---\n## 【出力フォーマット】\n[HEADER]\ncompany=\ncode=\nbusiness_description=\nannouncement_date=\nresult_label=\n\n[BASIC_INFO]\nmarket_cap=\nper=\npbr=\nyield=\n\n[CHART_DATA]\ndata=\n\n[KPI_HEADER]\nconsensus_label=\n\n[KPI_1]\ndisplay_label=売上高\nvalue=\nyoy=\nconsensus=\njudgment=\nreason=\n\n[KPI_2]\ndisplay_label=営業利益\nvalue=\nyoy=\nconsensus=\njudgment=\nreason=\n\n[KPI_3]\ndisplay_label=純利益\nvalue=\nyoy=\nconsensus=\njudgment=\n\n[GUIDANCE]\nstatus=\ncol1_header=\ncol2_header=\nsales_1=\nsales_2=\nop_1=\nop_2=\nop_progress=\ndiv_1=\ndiv_2=\nreason=\n\n[MARKET_REACTION]\npre_price_label=\npre_price=\npost_price_label=\npost_price=\nprice_change=\nreaction_summary=\n\n[NEXT_ACTIONS]\npoint_1=\npoint_2=",
                    //memberpresen: "目的：\n「決算動画」用の“メンバー向け画像1枚”に載せる要点の【根拠テキスト】を日本語で生成する。\n助言は行わず、事実と条件のみを簡潔に要約する。\n\n出力スタイル：\n・ですます調ではなく常体。短文・箇条書き中心。1行は短く。\n・主語は会社名やテーマ名で明示。推測語（〜だろう、〜と思われる）は禁止。\n・「情報提供のみ」「投資助言ではない」を末尾に記載。\n\n出力の大枠（順序固定／各ブロック3〜5行以内）：\n1) 直近業績：直近四半期・累計の要点と増減の主因\n2) 変化の核：構造・需要・資本の観点で“何が変わったか”を3点\n   - 構造（例：製品ミックス／コスト／稼働）\n   - 需要（例：地域・顧客・用途のモメンタム）\n   - 資本（例：配当方針／自己株買い／分割・希薄化）\n3) 見通し：通期方針や前提の変化（想定条件・投資計画・受注残など）\n4) 次に見る指標：次回決算までの「条件→シグナル」型チェック項目を4〜6点\n5) 近いイベント：決算／説明会／製品発表など日付つきで2〜3件\n6) 付記（任意）：価格スナップ（終値・評価倍率・簡易レンジ）※事実のみ\n7) 取得日と免責：「取得日：YYYY/MM/DD」「情報提供のみ（投資助言ではない）」\n\n可変ルール（資料に無い項目の扱い）：\n・開示が無い項目は書かない。推測で埋めない。\n・業種により見出し語を置換可（例：金融なら「直近業績」内で“利鞘・与信・自己資本”などに言い換え）。\n\n出力フォーマット（例／見出しだけ固定）：\n# 直近業績\n- …\n# 変化の核\n- 構造：…\n- 需要：…\n- 資本：…\n# 見通し\n- …\n# 次に見る指標\n- …\n# 近いイベント\n- …\n# 付記（任意）\n- …\n（取得日：YYYY/MM/DD／情報提供のみ）",
                    //menberpresenmade: "目的：\n「決算動画」用の“メンバー向け要点画像（1枚）”を自動生成する。\n\nページ仕様：\n・スライド1枚のみ。横長（16:9）。余白広め。文字は短く。\n・要素の並びは上→下で一方向スクロール的に読む。\n\nコンテンツの並び（順序固定）：\nA) タイトル帯（1行）\n　【決算サマリ】《会社名（コード）》／《期・四半期》\nB) 直近業績（ミニ表×1）\n　・「売上」「利益」「利益率」など“主要KPIのスナップ”を大きな数字のレイアウトでまとめる\n　・各KPIは“前期比／前四半期比”の方向だけ矢印や＋−で示す（数値は最小限）\nC) 変化の核（アイコン付き箇条書き 3点）\n　・見出しだけ固定：「構造」「需要」「資本」\n　・各1行で“何が変わったか”を要約（長文禁止）\nD) 見通し（2列の簡易表）\n　・左列に項目ラベル（例：通期方針／前提条件／投資計画／受注・残高）\n　・右列に一言要約（据置・上方・下方／主要理由など）\n　・資料に無い行は省略する\nE) 次に見る指標（チェックリスト 4〜6項）\n　・「条件→シグナル」形式の短句（例：B/B<1.0が2Q継続→減速シグナル）\n　・チェックボックス風の記号で視認性を上げる\nF) 近いイベント（タイムライン/箇条書き 2〜3件）\n　・日付→イベント名→一言（決算・説明会・製品発表など）\nG) フッター（1行）\n　・《取得日 YYYY/MM/DD》・《情報提供のみ（投資助言ではない）》を右下に\n\n図・表の使い分けルール：\n・Bは“ミニ表”で全体像を一望。棒グラフ等は使わない（1枚の情報量を抑える）。\n・Dは“2列の簡易表”で見通しを並べる。セルは3〜5行に収める。\n・Eは“チェックリスト”として横並び or 2段組の箇条書き。短く。\n・Fは“タイムライン”か箇条書き。日付を先頭にして視認性を確保。\n\n可変ルール（業種差分・未開示対応）：\n・業種によりBのKPI見出しは置換可（例：金融＝純利益・与信・自己資本、SaaS＝ARR・解約率など）。\n・資料に無い項目は丸ごと省略し、上下の余白で整える。推測で補完しない。\n\n入力データの受け取り方：\n・NotebookLMで作った「根拠テキスト」の見出し単位（直近業績／変化の核／見通し／次に見る指標／近いイベント）をそのまま差し込む。\n・会社名・コード・期・四半期・取得日はプレースホルダで渡される前提。\n\n出力のゴール：\n・スマホで一瞥して“何が起きた・何が変わった・次に何を見るか”が分かる1枚。\n・助言や評価は含めない。事実と条件のみ。",
...
表紙、企業概要、事業内容、企業の特長・強み、課題・懸念材料、市場の将来性、業績（直近実績）、業績（累積実績）、業績（次期予想）、業績推移、財務状況、需給環境、株価の妥当性、IPOの基本情報、投資判断、ポジティブ要因、ネガティブ要因、まとめ`,
            ...
            content_creation: {
                ...
                copyTexts: {
                    ...
                    textCorrection: "以下の文章の内容を変えずに３０００字程度にし、そのままwordpressにアップできるように、見出しや太字の強調、表を追加して見やすく修正してください。\n文章の表記などの変更は構いませんが、記載している内容を減らさないでください。\n\n--記事内容--\n{{textbox2}}",
                    tagGeneration: "以下の文章をそのままwordpressにアップできるように、見出しや太字の強調、表を追加して見やすく修正してください。\n文章の表記などの変更は構いませんが、記載している内容を減らさないでください。\nヘッダはh1、h2のみを使用してください。\n\n--記事内容--\n{{textbox2}}",
                    youtubeDescription: `\n\n📄 テキスト版はこちら
👉 ブログ記事：{{textbox2}}

動画で触れきれなかった補足や要点をまとめています。`
                }
            },
            youtube_posting: {
                label: 'Youtube投稿内容',
            ...
            earnings_summary: {
                ...
                copyTexts: {
                    ...
                    voice: "複数銘柄の決算内容をまとめて分析してください。{{VoiceNote_Principle}}{{VoiceNote_Read}}{{readingNote}}{{VoiceNote_Basic}}{{VoiceNote_Ks}}{{VoiceNote_Size}}{{VoiceNote_tatoe}}{{VoiceNote_kaihi}}",
            ...
            ai_education: {
                    ...
                    { 
                        ...
                        services: [{ 
                            ...
                            buttons: [
                                ...
                                { label: "ショート音声", copyId: "shortVoice" },
                ...
                copyTexts: {
                    voice: `【目的】
この音声は「AI教養ラボ」のYoutube動画用です。
知的で静かなトーンを基調に、聴き手が落ち着いて考えられるナレーションを生成してください。
聞き手に情報を渡すことが目的で、何かを促したり動かすような目的はありません。

【構成ルール】
・冒頭は説明から入らず、共感を誘う一言で始めてください。
　例：「〜って、不思議ですよね」「〜してしまうこと、ありませんか？」
・冒頭10〜15秒は、リスナーが「自分のことだ」と感じるような日常的な導入にしてください。
・その後でテーマの核心に静かに移行し、理屈ではなく“気づき”の流れを意識してください。
・全体のテンポはゆっくりで、穏やかで知的な雰囲気を保ちます。
・音量の起伏は最小限にし、抑揚よりも“間”で静けさを演出してください。
・語尾は下げ調子で終わり、余韻を残すように締めてください。
・感情表現（強調・驚きなど）は避け、思考に寄り添うニュートラルな語りにしてください。
・締めくくりは「〜とされている」「〜である」といった事実・気づきで静かに終わります。
・「いかがでしたか」「考えてみてください」などの問いかけで終えないでください。
・相づち（ええ、ああ、なるほど）や挿入句（「例えば」「つまり」など）、まとめや結論は極力減らし、淡々と話題を進めてください。
・「次に」「ということは」「まとめると」などの、話を展開する接続詞は使わず、静かに話題を進めてください。

【文体】
・文体は常体（〜である、〜とする）で統一。
・説明ではなく、理解と内省を促す口調。
・専門用語は使いすぎず、誰でも理解できる表現に言い換えてください。

【長さ】
・音声時間は約3〜4分（原稿文字数700〜850字想定）。
・導入60字以内、結びは1回のみ。

【テーマ例】
心理・脳科学・哲学・人間行動・感情・思考など、「内面を考える」テーマを想定。

【禁止事項】
・同じ語句・語尾を繰り返す
・元資料や出典への言及
・強調語（「とても」「絶対に」など）の連続使用
・メタ発話（「この資料をもとに」など）
・章の見出しや章番号の使用
・結論を繰り返すことで強調しない

【全体トーン】
静か、知的、穏やか。NHK教育・ドキュメンタリー風の落ち着いた語り。
`,
                    shortVoice: `【目的】
この音声は「AI教養ラボ」のYoutube動画用です。
知的で静かなトーンを基調に、聴き手が落ち着いて考えられるナレーションを生成してください。
...
・文体・構成を毎回同一形式で保つこと（全動画共通フォーマット）`,
                    presentation: `・イラスト提案は抽象的（人、波、光など）。
・視覚的に理解できるように図や画像を多く活用する。`
            ...
            video_completion: {
                ...
                copyTexts: {
                    ...
                    notePost: `【特典｜保存版({{actualToday}})】動画の分析データ\n\n■ 対象動画：{{videoTitle}}\n■ 動画本編： {{videoUrl}}\n\n👇 動画の要点が一目でわかる特典画像はこちら\n\n— ※数値は動画公開時点のもの。 ※投資助言ではない。`,
                    postDescription: `---\n✅ 本日の実戦用データ配布中！\n▼ データ（note記事） ： {{postUrl}}\n---`,
                    postComment: `👇動画の要点が一目でわかる特典画像（保存用）はこちら👇\n👉 note記事： {{postUrl}}\n※ 特典画像（保存用）はnoteメンバーシップ登録で閲覧できます`,
  ...
  if (keys.length > 0) {
    ...
    getTemplateVariables: function(analysisKey) {
        ...
        this.dom.youtubeContent = document.getElementById('youtube-content');
        ...
        const baseVars = {
            ...
            earningsTimingRadio: form.earningsTiming,
            afterHours,
            VoiceNote_Ks,
            companyName,
            companyNamePrefix,
        ...
        if (analysisKey === 'market_term_rank') {
            ...
            Object.assign(baseVars, {
                termDirectionLabel,
                strategy,
                weeklyRankUrl: form.termDirection === 'up' ? 'https://www.nikkei.com/marketdata/ranking-jp/price-rise-weekly/?market=G_TP' : 'https://www.nikkei.com/marketdata/ranking-jp/price-drop-weekly/?market=G_TP',
                monthlyRankUrl: form.termDirection === 'up' ? 'https://www.nikkei.com/marketdata/ranking-jp/price-rise-monthly/?market=G_TP' : 'https://www.nikkei.com/marketdata/ranking-jp/price-drop-monthly/?market=G_TP',
    ...
    getCopyText: function(analysisKey, copyId, isShort = false, isPriority = false, voiceLength = 'default') {
        ...
        if (['market_stock', 'market_earnings', 'market_buy_analysis', 'news'].includes(analysisKey)) {
             if ((actualCopyId === 'voice') && variables.noPriceChecked) {
                 // 時間外チェックなし、株価なしチェックあり、株価変動指定なし、引け後選択の場合の特別メッセージ
                 if (!variables.afterHoursChecked && variables.direction === '' && variables.earningsTimingRadio === 'の引け後') {
                     finalText += '\n・株価の動きについては触れないでください。（{{today}}の引け後に発表された決算は、{{today}}の株価に影響しないため）';
                 } else {
                     finalText += '\n・株価の具体的な数字は発言しないでください。ただし、上昇や下落、ストップ高、ストップ安、大幅などという表現であれば可。';
                 }
             }
        ...
        // 追加されたメッセージ内の変数も置換
        finalText = this.replaceVariables(finalText, variables);
    ...
    clearYoutubeFields: function() {
        const fields = ['youtube-title', 'youtube-content', 'youtube-memo', 'youtube-url', 'youtube-new-title'];
